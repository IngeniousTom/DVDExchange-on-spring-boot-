...
<body ng-app="pageApp" ng-cloak data-page-name="takenDisks" ng-init="numberOfDisks='${TakenDisks.size()}'">
<div class="main_container">
    ...
    <div id="workspace" class="workspace">
        <div class="main_space" onmouseover="updateSACheckboxStatus()"
             onclick="updateSACheckboxStatus(); updateSelectedRowCounter(); checkForValueButttonConditions($('#give-back-commit-button'))">
            <div class="h-centeralize table-width page-description">
                <span>Взятые напрокат диски</span>
            </div>
            <div id="tableSpaceCtrl" ng-controller="tableSpaceCtrl">
                ...
                <div class="h-centeralize table-width">
                    <div class="bottom_line">
                        <div ng-show="(numberOfDisks < 1)">У вас нет взятых напрокат дисков</div>
                    </div>
                    <div id="tblock" class="tblock mCustomScrollbar" data-mcs-theme="md">
                        <table id="scrollTable" class="inner_table w2" data-select-mode="none">
                            <c:forEach items="${TakenDisks}" var="disk" varStatus="loop">
            ...
            <div id="buttonSpace" class="button_space">
                <ul>
                    <li>
                        <md-button class="md-primary md-raised md-hue-3" onclick="giveDiskBack(this.id)" ng-disabled="(numberOfDisks < 1)">Вернуть диск</md-button>
                    </li>
                </ul>
                <div class="counterHolder" ng-init="borrowLimit='${BorrowLimit}'; diskCount='${BorrowCount}'">
                    <b>Всего взято напрокат: </b><span id="diskCounter" class="counter" ng-bind="diskCount"></span><b> из </b><span
                        class="counter">${BorrowLimit}
                    ...
                    <span id="limitMessage" class="counter" ng-if="diskCount >= borrowLimit">(limit!)
                    <md-tooltip
                            md-direction="top" ng-cloak>Превышен лимит дисков! Чтобы взять новый диск - верните один из взятых ранее!</md-tooltip>
                    </span>
                </div>
            </div>
            <div id="workspaceDivider" class="workspace_divider" ng-controller="controlPanelCtrl"><%--Область для работы с записями таблицы дисков--%>
                <div id="deleteContextField" class="delete_context_field">
                    <div id="rowsCountField" class="rows_count_field">
                        <span id="diskInfoField"><b>Всего выбрано: </b></span>
                        <span id="selectedRowsCounter" class="counter">0</span>
                        <div id="deleteContextButtonSpace" class="delete-context_button-space">
                            <md-button id="give-back-commit-button" class="md-primary md-raised md-hue-3" onclick="commitButtonPressed()">Вернуть
                            </md-button>
